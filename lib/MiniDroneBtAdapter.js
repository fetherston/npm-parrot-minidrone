...
// https://github.com/Parrot-Developers/arsdk-xml/blob/ab28dab91845cd36c4d7002b55f70805deaff3c8/xml/common.xml
const MD_CLASSES = {
    PILOTING: 0x00,
    SPEED_SETTINGS: 0x01,
    ANIMATION: 0x04,
...
class MiniDroneBtAdapter extends EventEmitter {
     ...
     * @param {String} deviceType Optional project class to write to
     * @return {buffer}      A freshly created Buffer stream
     */
    createBuffer(uuid, args = [], deviceType = MD_DEVICE_TYPE) {
        const buffArray = [MD_DATA_TYPES.DATA, ++this.steps[uuid] & 0xFF, deviceType];
     ...
     * @param  {integer} rotationSpeed The max rotation speed from (50°-360° for Airborne Cargo / 50° - 180° for Mambo)
     * @return {undefined}
     */
    ...
    /**
     * Convenience method for sending the headlight animation command
     * Can be fixed, blinking, or oscillated
     * @param  {string} animation The desired headlight animation, can be 'flash', 'blink' or 'oscillate'
     * @return {undefined}
     */
    writeHeadlightAnimation(animation = 'flash') {
        ...
        const buffer = this.createBuffer(
            COMMAND_KEY, [
                0x18, // class
                MD_METHODS.LIGHT_STATE, // method
                0x00, // ?
                lightAnimation[animation], // argument
                0x00,
                0x00,
                0x00,
            ], MD_COMMON
        ...
        Logger.info(`Headlight animation command called with ${animation} argument.`);
    }
