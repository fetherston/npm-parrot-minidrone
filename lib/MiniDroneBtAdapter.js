...
const DRONE_PREFIXES = ['RS_', 'Mars_', 'Travis_', 'Maclan_', 'Mambo_'];
...
class MiniDroneBtAdapter extends EventEmitter {
    ...
    writeMaxVerticalSpeed(verticalSpeed) {
        const buffer = this.createBuffer(COMMAND_KEY, [MD_CLASSES.PILOTING_SETTINGS, MD_METHODS.MAX_VERTICAL_SPEED, 0x00, verticalSpeed, 0x00]);
        this.write(COMMAND_KEY, buffer);
    ...
    writeMaxRotationSpeed(rotationSpeed) {
        const buffer = this.createBuffer(COMMAND_KEY, [MD_CLASSES.PILOTING_SETTINGS, MD_METHODS.MAX_ROTATION_SPEED, 0x00, rotationSpeed, 0x00]);
        this.write(COMMAND_KEY, buffer);
